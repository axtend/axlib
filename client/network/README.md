...
In Axlib, the keys are based on the ed25519 curve.

From a node's public key, we can derive its *identity*. In Axlib and libp2p, a node's
...
# Axlib's network protocol

Axlib's networking protocol is based upon libp2p. It is at the moment not possible and not
...
Axlib uses the following standard libp2p protocols:
...
connection we optionally keep an additional substream for all Axlib-based communications alive.
This protocol is considered legacy, and is progressively being replaced with alternatives.
This is designated as "The legacy Axlib substream" in this documentation. See below for
more details.
...
Axlib uses a component named the **peerset manager (PSM)**. Through the discovery
mechanism, the PSM is aware of the nodes that are part of the network and decides which nodes
we should perform Axlib-based communications with. For these nodes, we open a connection
if necessary and open a unique substream for Axlib-based communications. If the PSM decides
...
Axlib substream. Additionally, the handshake message is hardcoded to be a single 8-bits
